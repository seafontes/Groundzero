...
def count_elements(city, element):  # returns the number of a determined type of element,ex.: count_elements(city, DEAD)
    return str(np.count_nonzero(city[0:order, 0:order].astype(int) == element))
...
def initialize_city():  # returns a numpy matrix of an order specified on section 2
    city = np.random.rand(order+1, order+1)
    ...
    city[0:order, order] = city[0:order, 0]
    city[order, 0:order] = city[0, 0:order]
    city.astype(float)
    return city
